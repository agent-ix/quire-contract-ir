...
| FND-003 | low | The lowering paragraph lacked an EARS subject and trigger; fixed with an explicit `When … the lowerer shall` form. | FR-038 Behavior |
...
- IDs FR-038, TC-048 and TC-050 are the next free identifiers; criteria use
  `FR-038-AC-N`; no duplicates.
- Inputs, outputs, closed result sums and every refusal code are enumerated.
- Error paths: every I04 refusal code and every lowering non-success
  disposition has an acceptance criterion.

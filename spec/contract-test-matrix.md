...
| FR-031 | FR-031-AC-1 through FR-031-AC-4 | TC-042 | 🚧 AC-1 and AC-2 implemented through native replay PR #92 and codegen corpus PR #47: module dispatch refuses cross-family approximation and every generated artifact binds its provenance. AC-3 planned (issue #137): the evaluated witness the criterion names is never produced from a Kani transcript, and `tests/kani_replay.rs:239` supplies `reconstruct` as `\|_\|`, which discards the packet and returns a constant input. AC-4, the witness vocabulary AC-3 consumes, is added and implemented (issue #137): a transcript-parsed witness carries untyped concrete values and is typed only against a declared schema, refusing on arity, width or backend-comment disagreement |
...
| TC-042 | Versioned bounded-Kani profile, finite input/outcome firewall, module dispatch, provenance, and native counterexample replay conform | Integration | P0 | FR-029, FR-030, FR-031 | 🚧 profile, firewall, dispatch and provenance implemented across the shared, arithmetic, graph and collection suites against the integrated codegen corpus at `73c82ad`; the counterexample-replay population is planned (issue #137) because no case carries a witness extracted from a Kani transcript |

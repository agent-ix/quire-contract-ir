...
lowest-digest node sharing the name. QSpec FR-322 orders both before any frame
...
| FR-038-AC-21 | A node whose declared `qualified_name` differs from its nominal preimage's `qualified_declaration` refuses as `invalid_package` with cause `declaration-nominal-mismatch` at that node; two nodes declaring one name refuse as `ambiguous_declaration` with cause `ambiguous-name` at the lower-digest of the two; and a package carrying both defects refuses for the nominal mismatch. | Test (TC-048) |
